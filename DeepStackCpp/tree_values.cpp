...
//
//void tree_values::_fill_ranges_dfs(Node & node, Tf2 & ranges_absolute)
//{
...
//	Tf2 strategy_to_check = node.strategy;
//
//	if (node.current_player != chance)
//	{
//		std::array<DenseIndex, 1> dims = { 1 };
//		Tf1 checksum = strategy_to_check.sum(dims);
//		assert(((Tf1)(strategy_to_check > 0.f).all())(0));
//		assert(((Tf1)(strategy_to_check < 1.001f).all())(0));
//		assert(((Tf1)(checksum > 0.999f).all())(0));
//		assert(((Tf1)(checksum < 1.001f).all())(0));
//	}
//
//	assert(((Tf1)(node.ranges_absolute >= 0).all())(0));
//	assert(((Tf1)(node.ranges_absolute < 1).all())(0));
//
//	//--check if the range consists only of cards that don't overlap with the board
//
//#ifdef _DEBUG
//	Tf2 hands_mask = _cardTools.get_possible_hand_indexes(node.board);
//	hands_mask.resize(1, card_count);
//	hands_mask = Util::ExpandAs(hands_mask, node.ranges_absolute);
//
//	assert(SumSc((hands_mask * node.ranges_absolute)) == SumSc(node.ranges_absolute)); // Checking that multiplication with possible hands range did not changed anything.
//
//	//impossible_hands_mask.resize(1, card_count);
...
//	//vector<Tf2&> children_ranges_absolute(actions_count);
//	Tf2 child_range(players_count, card_count);
//
//	//--chance player
//	if (node.current_player == chance)
//	{
...
//		{
//			child_range.chip(0, 0) = node.ranges_absolute.chip(0, 0) * node.strategy.chip(i, 0);
//			child_range.chip(1, 0) = node.ranges_absolute.chip(1, 0) * node.strategy.chip(i, 0);
//
//			//--go deeper
...
//			child_range.row(currentPlayerIndex) = node.ranges_absolute.row(currentPlayerIndex) * node.strategy.chip(i, 0);
//
//			//--go deeper
//			_fill_ranges_dfs(*node.children[i], child_range);
//		}
...
//
//		Tf2 values = Tf2::Zero(players_count, card_count);
//
...
//		//--multiply by the pot
//		values *= node.pot;
//		node.cf_values = Tf2(values);
//		node.cf_values_br = Tf2(values);
//	}
//	else
//	{
//
...
//		node.cf_values_br = Tf2::Zero(players_count, card_count); 
//
//		for (size_t i = 0; i < actions_count; i++)
//		{
...
//			else
//			{
//				node.cf_values.row(current_player) += child_node->cf_values.row(current_player) * node.strategy.chip(i, 0);
...
//	node.cfv_infset = Tf1(players_count);
...
//	node.cfv_br_infset = Tf1(players_count);
...
//}
//
//void tree_values::compute_values(Node& root, Tf2* starting_ranges)
//{
//	Tf2 range;
//
...
//	{
//		range = Tf2::Constant(players_count, card_count, 1.0f / card_count);
//	}
//	else
//	{
...
//#ifdef _DEBUG
//
//	Tf1 checksum = range.rowwise().sum();
