...
TEST_CASE("cfr_gadget_Ks_2_iter")
{
	...
	ArrayX opponent_cfvs = ArrayX(card_count);
	Util::Print(opponent_cfvs);
	opponent_cfvs.setConstant(0.2f);
	opponent_cfvs(2) = 0;

	ArrayX board = converter.string_to_board("Ks");
	ArrayX player_range = tools.get_uniform_range(board);

	cfrd_gadget gadget(board, player_range, opponent_cfvs);
	ArrayX cur_opponent_cfvs = ArrayX(opponent_cfvs);
	cur_opponent_cfvs.setConstant(0);

	ArrayX op_range = gadget.compute_opponent_range(cur_opponent_cfvs);
	...
	cur_opponent_cfvs << 240, 240, 0, 960, -720, -720 ;
...
TEST_CASE("cfr_gadget_root_3_iter")
{
	...
	cur_opponent_cfvs << 119.7473f, 118.7914f, -15.3321f, -15.4158f, -149.6880f, -149.7141f;
	op_range = gadget.compute_opponent_range(cur_opponent_cfvs);
	REQUIRE(op_range(0) == Approx(1.0).epsilon(myEps));
	REQUIRE(op_range(1) == Approx(1.0).epsilon(myEps));
	...
	cur_opponent_cfvs << 249.8833f,  249.3737f, -157.0048f, -157.2358f,  -55.4940f,  -55.2005f;
	op_range = gadget.compute_opponent_range(cur_opponent_cfvs);
	REQUIRE(op_range(0) == Approx(1.0).epsilon(myEps));
	REQUIRE(op_range(1) == Approx(1.0).epsilon(myEps));
	REQUIRE(op_range(2) == Approx(1.2904e-09f).epsilon(myEps));
